/**
 * @fileoverview Instagram OAuth callback handler.
 * Exchanges the authorization code for an access token, retrieves the user's
 * Facebook pages, finds the linked Instagram Business Account, and stores
 * the credentials in Vercel KV.
 *
 * @route GET /api/auth/instagram/callback
 */

import { setToken } from '@/lib/kv';
import type { StoredToken, FacebookPagesResponse, InstagramBusinessAccountResponse } from '@/lib/types';
 ...
 * @returns {Promise<string | null>} The Instagram Business Account ID, or null if none linked
 * @throws {Error} If the API request fails
 */
async function getInstagramBusinessAccount(
  pageId: string,
  accessToken: string
): Promise<{ igBusinessAccountId: string; pageName: string } | null> {
  ...
  if (!response.ok) {
    throw new Error('Failed to retrieve Instagram Business Account');
  }
...
/**
 * GET handler - Processes the Instagram OAuth callback.
 *
  ...
  try {
    ...
    if (error || errorReason) {
      ...
      return Response.redirect(
        new URL(`/?error=Instagram+authorization+denied`, appUrl),
        302
      );
    }
    ...
    let connectedAccount: StoredToken | null = null;
    ...
    for (const page of pages) {
      ...
      if (igData) {
        connectedAccount = {
          ...
          pageId: page.id,
          igBusinessAccountId: igData.igBusinessAccountId,
    ...
    if (!connectedAccount) {
      return Response.redirect(
        new URL('/?error=No+Instagram+Business+Account+linked+to+any+page', appUrl),
        302
      );
    }

    // Store token in KV
    await setToken('instagram', connectedAccount);
    console.log('[Instagram Callback] Successfully connected Instagram account');

    return Response.redirect(new URL('/?connected=instagram', appUrl), 302);
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : 'Unknown error';
    console.error('[Instagram Callback] Error:', errorMessage);
    return Response.redirect(
